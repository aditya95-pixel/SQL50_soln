...
```sql
select distinct c1.visited_on,(select sum(c2.amount) from customer as c2 where c2.visited_on
between date_sub(c1.visited_on,interval 6 day) and c1.visited_on) as amount,round((select sum(c2.amount) from customer as c2 where c2.visited_on
between date_sub(c1.visited_on,interval 6 day) and c1.visited_on)/7,2) as average_amount
from customer as c1
where date_sub(c1.visited_on,interval 6 day) in (select distinct visited_on from customer)
...
group by t.id
having num=(select max(t1.num) from (
select t.id,sum(t.cnt) as num
from (
    select requester_id as id,count(accepter_id) as cnt
    from requestaccepted
...
from insurance as i
where (lat,lon) not in (select lat,lon from insurance where i.pid<>pid)
and tiv_2015 in (select tiv_2015 from insurance where i.pid<>pid);
```
...
from (
    select d.name as department,e.name as employee,e.salary as salary,dense_rank() over (partition by e.departmentid
